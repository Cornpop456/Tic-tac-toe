...
function checkBoard(row, col) {
  if (boardState[row].every(function (cell) {
    return cell === currentMove;
  })) {
    return currentMove;
  } else if (boardState.every(function (row) {
    return row[col] === currentMove;
  })) {
    return currentMove;
  } else if (diagonals.some(function (diagonal) {
    return diagonal.every(function (_ref) {
      var _ref2 = _slicedToArray(_ref, 2),
          row = _ref2[0],
          col = _ref2[1];

      return boardState[row][col] === currentMove;
    });
  })) {
    return currentMove;
  } else if (boardState.every(function (row) {
...
function renderBoard() {
  ...
  moveElement.innerText = winner ? "Winner is ".concat(winner) : "Next Move: ".concat(currentMove);
